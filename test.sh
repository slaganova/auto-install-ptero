...
install_options(){
	...
	5 ) installoption=5
        ...
        9 ) installoption=9
            ...
            ;;
       10) installoption=10
            output "[10] Установка панели Jexactyl с использованием IP-адреса (без HTTPS)."
            ;;
...
nginx_config_ip() {
    output "Настройка Nginx для использования IP-адреса..."
    ...
    ' | sudo tee /etc/nginx/sites-available/jexactyl-ip.conf >/dev/null 2>&1

    ln -s /etc/nginx/sites-available/jexactyl-ip.conf /etc/nginx/sites-enabled/jexactyl-ip.conf
...
EOF
    ...
    php artisan p:environment:setup -n --author=$email --url=http://$SERVER_IP --timezone=America/New_York --cache=redis --session=database --queue=redis --redis-host=127.0.0.1 --redis-pass= --redis-port=6379
    php artisan p:environment:database --host=127.0.0.1 --port=3306 --database=panel --username=jexactyl --password=$password
    ...
    output "Creating panel queue listeners..."
(crontab -l ; echo "* * * * * php /var/www/jexactyl/artisan schedule:run >> /dev/null 2>&1") | crontab -
(crontab -l ; echo "0 0 * * * php /var/www/jexactyl/artisan p:schedule:renewal >> /dev/null 2>&1") | crontab -
...
case $installoption in 
    ...
    10)  repositories_setup
       required_infos
       output "Введите IP-адрес сервера (например, 192.168.0.1):"
       read -r SERVER_IP
       firewall
       install_dependencies
       install_jexactyl_2
